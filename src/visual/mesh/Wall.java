...
{
	public Wall(int direction, WallType type, boolean low, boolean wall)
	{
		...
		for(int i = 0; i < 7; i++)
		{
			int in = type.ground[(i % 6)];
			lkl[i] = edges[(i + direction) % 6].mult(1 - (in * Scale.HEX_EDGE_IN)).add(sl);
			if(in > 0)
				lku[i] = edges[(i + direction) % 6].mult(1 - Scale.HEX_EDGE_IN);
			else if(wall)
				lku[i] = edges[(i + direction) % 6];
			else
				lku[i] = edges[(i + direction) % 6].mult(1 - Scale.HEX_EDGE_FLOOR);
		}
